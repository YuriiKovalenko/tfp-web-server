...
let RegionService = class RegionService {
    ...
    getCountryByText(text) {
        ...
        return this.getCountryList().pipe(operators_1.map(countries => countries
            .filter(country => country.name.toLowerCase().includes(test) ||
            country.nativeName.toLowerCase().includes(test) ||
            country.altSpellings.some(sp => sp.toLowerCase().includes(test)))
            .slice(0, 6)
            .map(({ name, nativeName, flag }) => ({ name, nativeName, flag }))));
    }
    getCityByTextAndCountryName(text, country) {
        return this.getCountryCodeByName(country).pipe(operators_1.switchMap(code => this.getCitiesByCountryCodeAndText(code, text)), operators_1.map(result => result.data.geonames
            .slice(0, 6)
            .map(({ toponymName, name }) => ({ toponymName, name }))));
    }
};
